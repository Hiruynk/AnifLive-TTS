...
# Changes: TensorRT 11 execution, persistent bindings, local-only assets, and API sampling controls.
...
from aniflive_tts.backend.compatibility import ensure_tensorrt11
...
def sample_topk(topk_values, topk_indices, temperature=1.0, top_k=None, top_p=1.0):
    ...
    still supports a deterministic top-k cap and nucleus filtering within that
    candidate set, without requiring a non-TensorRT model fallback.
    """
    # Support both CPU and GPU tensors
    device = topk_values.device
    if top_k is not None:
        try:
            candidate_count = max(1, min(int(top_k), int(topk_values.shape[-1])))
        except (TypeError, ValueError):
            candidate_count = int(topk_values.shape[-1])
        topk_values = topk_values[..., :candidate_count]
        topk_indices = topk_indices[..., :candidate_count]
    if temperature != 1.0:
    ...
    if top_p is not None and float(top_p) < 1.0:
        # ``torch.topk`` returns values in descending-logit order, so its
        # softmax probabilities have the required descending order too.
        cumulative = torch.cumsum(probs, dim=-1)
        keep = (cumulative - probs) <= max(float(top_p), 0.0)
        keep[..., 0] = True
...
class TRTModule:
    def __init__(self, engine_path, device="cuda", stream=None):
        ...
        if self.engine is None:
            raise RuntimeError(f"TensorRT 11 could not deserialize engine: {engine_path}")
        self.context = self.engine.create_execution_context()
        if self.context is None:
            raise RuntimeError(f"TensorRT 11 could not create an execution context: {engine_path}")
        ...
        print(
            f"  - Loaded {os.path.basename(engine_path)} with TensorRT "
            f"{self.tensorrt_version}: Inputs={self.input_names}, Outputs={self.output_names}"
        )
    ...
    def __call__(self, inputs, outputs=None, sync=True):
        ...
        if missing:
            raise ValueError(f"Missing TensorRT inputs: {missing}")
        ...
        for name in self.input_names:
            ...
            held_tensors[-1] = tensor
            shape = tuple(tensor.shape)
            if self.is_dynamic[name] and self.bound_shapes.get(name) != shape:
                if not self.context.set_input_shape(name, shape):
                    raise ValueError(f"TensorRT rejected {name} shape {shape}")
                self.bound_shapes[name] = shape
                self.output_shapes.clear()
            address = tensor.data_ptr()
            if self.bound_addresses.get(name) != address:
                if not self.context.set_tensor_address(name, address):
                    raise RuntimeError(f"TensorRT failed to bind input {name}")
                self.bound_addresses[name] = address
        ...
        for name in self.output_names:
            ...
            if not unresolved and outputs.get(name) is None:
                ...
                if cached is None or tuple(cached.shape) != shape:
                    ...
                    cached = torch.empty(shape, dtype=self.tensor_dtype[name], device=out_device)
                    self.output_tensors[name] = cached
                outputs[name] = cached
            ...
            if output_tensor.dtype != self.tensor_dtype[name]:
                raise TypeError(
                    ...
                    f"{self.tensor_dtype[name]}"
                )
            address = output_tensor.data_ptr()
            if self.bound_addresses.get(name) != address:
                if not self.context.set_tensor_address(name, address):
                    raise RuntimeError(f"TensorRT failed to bind output {name}")
                self.bound_addresses[name] = address
            
        # 3. Execute
        if not self.context.execute_async_v3(stream_handle=self.stream.cuda_stream):
            raise RuntimeError("TensorRT 11 execute_async_v3 returned false")
        if sync:
...
class GPTSoVITS_TRT_Inference:
    def __init__(self, trt_dir, bert_path, device="cuda"):
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(
            bert_path, local_files_only=True, trust_remote_code=False
        )
    ...
    def infer(self, ref_wav_path, prompt_text, prompt_lang, text, text_lang,
              top_k=5, top_p=1.0, temperature=1.0, noise_scale=0.5, speed=1.0, output_path="out.wav", pause_length=0.3):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--trt_dir", default="onnx_export/example_v2proplus_fp16")
