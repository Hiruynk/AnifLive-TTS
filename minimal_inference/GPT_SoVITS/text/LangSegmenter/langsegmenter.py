...
# Keep the large fast-langdetect model outside the immutable application tree
# when deployed in Docker.  The default preserves upstream behaviour for
# standalone minimal-inference users, while deployments can point this at a
# persistent cache so an image update cannot trigger another first-request
# download.
from pathlib import Path
import fast_langdetect


def _configure_mecab_ko_dictionary() -> None:
    """Expose the argument contract expected by wordfreq's Korean tokenizer."""
    try:
        import mecab_ko_dic
    except ImportError:
        return
    if hasattr(mecab_ko_dic, "MECAB_ARGS"):
        return
    dictionary_path = getattr(mecab_ko_dic, "dictionary_path", None)
    if dictionary_path is None:
        return
    rc_path = "nul" if os.name == "nt" else "/dev/null"
    mecab_ko_dic.MECAB_ARGS = f'-r {rc_path} -d "{Path(dictionary_path).as_posix()}"'


_configure_mecab_ko_dictionary()


def _fast_langdetect_cache_dir() -> str:
    configured = os.environ.get("ANIFLIVE_TTS_FAST_LANGDETECT_CACHE")
    if configured:
        return str(Path(configured).expanduser())
    return str(Path(os.getcwd()) / "pretrained_models" / "fast_langdetect")


def _fast_langdetect_config():
    model_path = os.environ.get("ANIFLIVE_TTS_FAST_LANGDETECT_MODEL")
    if model_path:
        return fast_langdetect.infer.LangDetectConfig(
            custom_model_path=str(Path(model_path).expanduser()),
            allow_fallback=False,
        )
    return fast_langdetect.infer.LangDetectConfig(cache_dir=_fast_langdetect_cache_dir())


fast_langdetect.infer._default_detector = fast_langdetect.infer.LangDetector(
    _fast_langdetect_config()
)
