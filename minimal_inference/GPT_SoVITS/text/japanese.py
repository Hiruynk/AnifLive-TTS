...
try:
    ...
    # The source CSV is immutable application data.  Put the generated binary
    # in a separate cache when requested so Docker image updates do not force
    # the expensive MeCab dictionary compilation again.  The CSV hash makes a
    # changed source dictionary rebuild safely on the next start.
    USERDIC_CSV_PATH = os.path.join(current_file_path, "ja_userdic", "userdict.csv")
    USERDIC_CACHE_DIR = os.environ.get(
        "ANIFLIVE_TTS_JA_USERDIC_DIR", os.path.join(current_file_path, "ja_userdic")
    )
    os.makedirs(USERDIC_CACHE_DIR, exist_ok=True)
    USERDIC_BIN_PATH = os.path.join(USERDIC_CACHE_DIR, "user.dict")
    USERDIC_HASH_PATH = os.path.join(USERDIC_CACHE_DIR, "userdict.md5")
    # If the binary or its matching source hash is absent, rebuild it.
    if os.path.exists(USERDIC_CSV_PATH):
        source_hash = get_hash(USERDIC_CSV_PATH)
        cached_hash = None
        if os.path.exists(USERDIC_HASH_PATH):
            with open(USERDIC_HASH_PATH, "r", encoding="utf-8") as f:
                cached_hash = f.read().strip()
        if not os.path.exists(USERDIC_BIN_PATH) or source_hash != cached_hash:
