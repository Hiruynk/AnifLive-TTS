...
# g2pk2 hard-codes eunjeon on Windows and may invoke pip at import time.
# Use mecab-python3 with the packaged Korean dictionary. Unlike
# python-mecab-ko, this can coexist with wordfreq's Japanese tokenizer on
...
if os.name == "nt":
    ...
    class _WindowsMecab:
        def __init__(self):
            ...
            dictionary_path = mecab_ko_dic.dictionary_path.as_posix()
            self._tagger = MeCab.Tagger(f'-r nul -d "{dictionary_path}"')
    ...
    class win_G2p(G2p):
        def check_mecab(self):
            if importlib.util.find_spec("MeCab") is None:
                raise RuntimeError(
                    "mecab-python3 and python-mecab-ko-dic are required; "
                    "install dependencies before starting AnifLive-TTS"
                )

        def get_mecab(self):
            return _WindowsMecab()
