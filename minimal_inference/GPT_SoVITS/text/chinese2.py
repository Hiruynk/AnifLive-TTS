...
# G2PW needs a separate ONNX model bundle.  The portable deployment does not
# ship that optional bundle, and the original helper would otherwise try
# to download it on the first Chinese request.  Keep the text frontend fully
# offline by default and use pypinyin instead.  A maintainer who deliberately
# bundles G2PWModel can opt back in inside the container with
# ANIFLIVE_TTS_ENABLE_G2PW=1; this is unrelated to the TensorRT speech backend.
is_g2pw = os.environ.get("ANIFLIVE_TTS_ENABLE_G2PW", "0").strip().lower() in {
    "1",
    "true",
    "yes",
    "on",
