...
# Changes: inference-only torch.nn.Module integration and training dependency removal.
...
class Text2SemanticLightningModule(torch.nn.Module):
    def __init__(self, config, output_dir, is_train=True):
        ...
        if is_train:
            raise RuntimeError(
                "This AnifLive-TTS vendored module is inference-only; training is not supported"
            )
        self.config = config
        self.top_k = 3
