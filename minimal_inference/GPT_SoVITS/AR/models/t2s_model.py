...
# Changes: inference-only accuracy helper and removal of torchmetrics runtime dependency.
import math
...
class MulticlassAccuracy(nn.Module):
    """Inference-distribution replacement for torchmetrics' training-only helper."""
    ...
    def forward(self, predictions: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        candidates = predictions.topk(self.top_k, dim=1).indices
        valid = torch.ones_like(targets, dtype=torch.bool)
        if self.ignore_index is not None:
            valid = targets != self.ignore_index
        correct = candidates.eq(targets.unsqueeze(1)).any(dim=1) & valid
        denominator = valid.sum().clamp_min(1)
        return correct.sum().to(dtype=torch.float32) / denominator
