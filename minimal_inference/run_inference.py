...
# Changes: portable startup, V2ProPlus warmup, profiling, and sampling controls.
...
class GPTSoVITSInference:
        ...
        try:
            ...
            with torch.no_grad():
                ...
                if self.vq_model.is_v2pro:
                    dummy_sv = torch.zeros(
                        (1, 20480), dtype=dummy_spec.dtype, device=self.device
                    )
                    decode_kwargs["sv_emb"] = [dummy_sv]
    ...
    def infer(self, ref_wav_path, prompt_text, prompt_lang, text, text_lang,
        ...
        self.last_profile = {
            "reference_processing_seconds": t_ref_end - t_ref_start,
            "text_processing_seconds": total_text_time,
            "gpt_seconds": total_gpt_time,
            "sovits_seconds": total_sovits_time,
            "first_segment_seconds": t_first_segment,
            "total_seconds": total_inference_time,
            "audio_duration_seconds": total_audio_duration,
            "rtf": rtf,
            "semantic_tokens": int(total_gpt_tokens),
            "gpt_tokens_per_second": gpt_tps,
        }
