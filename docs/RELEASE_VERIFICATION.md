...
`RELEASE-METADATA-AnifLive-TTS-v1.2.0-<profile>.json` and
`SBOM-AnifLive-TTS-v1.2.0-<profile>.spdx.json` and
`TRIVY-AnifLive-TTS-v1.2.0-<profile>.json` release assets, then verify:

1. `source_commit` equals the v1.2.0 release commit.
...
release workflow from the final v1.2.0 commit. Evidence from an older workflow
