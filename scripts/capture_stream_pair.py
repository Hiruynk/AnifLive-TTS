...
def request(
    host: str, port: int, model: str, voice_profile: str, stream: bool
...
def main() -> int:
    ...
    full, _, _ = request(args.host, args.port, args.model, args.voice_profile, False)
    stream, headers, ttfa = request(
        args.host, args.port, args.model, args.voice_profile, True
    )
    ...
    result = {"ttfa_seconds": ttfa, "stream_pcm_bytes": len(stream)}
